class Drawer{
	...
	clear(){	
		this.get().clearRect(0, 0, this.canvas.width, this.canvas.height);
	}
	
	getTilesScreen(){
		return {
			width: Math.ceil(window.innerWidth / Vars.tileSize) + 2*Vars.tileBuffer,
			height: Math.ceil(window.innerHeight / Vars.tileSize) + 2*Vars.tileBuffer
		}
	}
}
